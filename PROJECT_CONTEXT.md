...
| `condition`, `set_name`, `notes` | text (optional) |
...
| `/events/[id]/new-listing` | Protected | Create listing form |

## Remaining roadmap

1. **Join event** — use `join_code` to associate users with events
2. **Home page** — replace Next.js starter with app landing
...
- **Schema-driven UI** — events use `start_date`/`end_date` (not `date`/`description`); listings use enums for `type` and `status`; only `active` listings show on event pages; owners update status on `/my-listings`; `card_ref` is required in DB and derived from `card_name.trim().toLowerCase()` for MVP; interests are one per user per listing with `message: null` for MVP
